  ...
  while (strat->Ll >= 0)
  {
    ...
    if (red_result == 1)
    {
      ...
      if (dynamic_algorithm)
      {
        ...
        SelectMonomial(strat->P.p, CurrentLPPs, strat->S, strat->sl, skel, ORD_HILBERT_THEN_DEG);
        //cout << "selected\n";
        ...
        for (int i = 0; i <= strat->Ll; ++i)
        {
          if ((strat->L)[i].p != NULL)
          {
            ...
            if (strat->L[i].sev != 0) strat->L[i].sev = p_GetShortExpVector(strat->L[i].p, dynR);
